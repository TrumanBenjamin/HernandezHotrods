...
router.get('/', async (req, res, next) => {
  try {
    const { rows } = await db.query(`
      ...
      LIMIT 50
    `);
    res.render('builds/index', { title: 'Custom Builds', builds: rows });
  } catch (err) {
    next(err);
  }
});

// GET /builds/:slug — detail page (alternating info/image blocks)
router.get('/:slug', async (req, res, next) => {
  try {
    ...
    // Pair each section with the photo at the same index (fallback to hero if no photo)
    ...
    // Any extra photos beyond the section count go in a small “More photos” gallery
